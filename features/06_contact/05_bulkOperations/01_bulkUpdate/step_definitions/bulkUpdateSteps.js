...
Then('user is on contact listing page',async function(){
    try {
...
//---Case 1: Verify that User can't able to see Update options on bulk operation when user doesn't have a right to Mass Update Contact----------------

Then('{string} button is not visible and logged in through {string}',async function(updateButton,adminUser) {
    try {
        ...
        const updateContactButton = await driver.findElements(By.xpath(`//button[text()=" ${updateButton} "]`));
        const updateContactButtonLength = await updateContactButton.length;
        if (updateContactButtonLength === 0) {
            console.log("As update contact button length is " + updateContactButtonLength + " so it is not displayed after disabling mass update contact right,so test case has been passed");
        } else {
            await assert.fail("As update contact button length is " + updateContactButtonLength + " so it is displayed after disabling mass update contact right,so test case has been aborted");
        }
        ...
        await screenshotObj.takeScreenshot(driver,screenshotPath+'updateButton_Visibility.png');
        await openSalesmatePageObj.openSalesmateLoginPage(driver,screenshotPath,'the {string} is on contact listing page');
        await performSalesmateLoginObj.performSalesmateLogin(driver, screenshotPath, adminUser, 'the {string} is on contact listing page');
...
//---Case 2: As a user, I should able to see the 'Update' option while clicking to the checkbox available on grid header of contact module------------------

Then('{string} button is visible and logged in through {string}',async function(updateButton,adminUser) {
    try {
        const allContactsCheckbox = await moduleElementsObj.findContactCheckbox(driver,1);
        await allContactsCheckbox.click();
        await driver.sleep(1500);
        ...
        const updateContactButtonLength = await updateContactButton.length;
        if (updateContactButtonLength > 0) {
            console.log("As update contact button length is " +updateContactButtonLength+ " so it is displayed after enabling mass update contact right,so test case has been passed");
        } else {
            await assert.fail("As update contact button length is " +updateContactButtonLength+ " so it is not displayed even after enabling mass update contact right,so test case has been aborted");
        }
        await openSalesmatePageObj.openSalesmateLoginPage(driver,screenshotPath,'the {string} is on contact listing page');
        await performSalesmateLoginObj.performSalesmateLogin(driver, screenshotPath, adminUser, 'the {string} is on contact listing page');
        await driver.sleep(2000);
    }catch(err) {
        await screenshotObj.takeScreenshot(driver,screenshotPath+'updateButton_Invisibility.png');
        await openSalesmatePageObj.openSalesmateLoginPage(driver,screenshotPath,'the {string} is on contact listing page');
        await performSalesmateLoginObj.performSalesmateLogin(driver, screenshotPath, adminUser, 'the {string} is on contact listing page');
        await driver.sleep(3000);
        await assert.fail(err);
...
//----------------Case 3: As a user, I should be able to Bulk update contacts from contacts listing screen--------------

When('user is able to Bulk update contacts from contacts listing screen',async function(dataTable) {
    try {
        ...
        for (let i = 0; i < dataTable.rawTable.length; i++) {
            ...
            //     console.log(emailOptOutUpdatedValue);
            //     const saveButton = await formElementsObj.findElementById(driver,screenshotPath,'btnSave','saveButton');
            //     await saveButton.click();
            //     await driver.sleep(1000);
            //     const notificationMsgElement = await driver.findElement(By.xpath('//span[@class="noty_text"]'));
        ...
        // const quickPageSmsOptOut = await driver.findElement(By.id('smsOptOut')).getAttribute('value');
