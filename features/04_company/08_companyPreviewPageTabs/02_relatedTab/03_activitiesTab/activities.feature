@skip_in_ci
Feature: Company Module > Preview Page > Related Tab > Activities
...
#------------------------ Case:6 (http://testrails.rapidops.com/index.php?/cases/view/C22716) --------------------------
  ...
  Scenario: As a user, Verify that Activity can be removed/ disassociate from contact by the remove option on Activity card
    When user is on preview page > related tab
    Then verify that Activity can be removed disassociate from 'icon-ic_company' by the remove option on Activity card
