...
const massUpdateElementsObj = require('../common/massUpdateElements');
...
//---Case 1: Verify that User can't able to see Update options under 'Actions' when user doesn't have a right to Mass Update Products----------------

Then('{string} is not visible and log in through {string}',async function(massUpdateProductButton,adminUser) {
    try {
        ...
        const massUpdateProductLink = await driver.findElements(By.xpath(`//a[text()="${massUpdateProductButton}"]`));
        const massUpdateProductLinkLength = await massUpdateProductLink.length;
        if (massUpdateProductLinkLength === 0) {
            console.log("As mass update product link length is " + massUpdateProductLinkLength + " so it is not displayed under 'Actions' after disabling mass update product right,so test case has been passed");
        } else {
            await assert.fail("As mass update product link length is " + massUpdateProductLinkLength + " so it is displayed under 'Actions' after disabling mass update product right,so test case has been aborted");
        }
...
//---Case 2: As a user, I should able to see the 'Update' option under 'Actions' when user have right to Mass Update Products------------------

Then('{string} is visible and log in through {string}',async function(massUpdateProductButton,adminUser) {
    try {
        const productIcon = await formElementsObj.findElementByXpath(driver,screenshotPath,'span','class','icon-ic_product','Product Icon');
        await productIcon.click();
        await driver.sleep(2000);
        ...
        const massUpdateProductLinkLength = await massUpdateProductLink.length;
        if (massUpdateProductLinkLength > 0) {
            console.log("As mass update product link length is " + massUpdateProductLinkLength + " so it is displayed under 'Actions' after enabling mass update product right,so test case has been passed");
        } else {
            await assert.fail("As mass update product link length is " + massUpdateProductLinkLength + " so it is not displayed under 'Actions' even after enabling mass update product right,so test case has been aborted");
        }
        await openSalesmatePageObj.openSalesmateLoginPage(driver,screenshotPath,'the {string} is on product listing page');
        await performSalesmateLoginObj.performSalesmateLogin(driver, screenshotPath, adminUser, 'the {string} is on product listing page');
    } catch(err) {
...
//----------Case 3: As a user,I should be able to update products in bulk from the mass update products page------------

When('user able to update products in bulk from the mass update products page',async function(dataTable) {
    try {
        let expectedNotification = 'Record(s) updated successfully';
        const actionsButton = await formElementsObj.findElementByVisibleText(driver,screenshotPath,'button',' Actions ');
        await actionsButton.click();
        await driver.sleep(1000);
        const massUpdateProduct = await formElementsObj.findElementByVisibleText(driver,screenshotPath,'a','Mass Update Products')
        await massUpdateProduct.click();
        await driver.sleep(1000);
        ...
        const listField = await massUpdateElementsObj.findListField(driver, 1);
        await listField.click();
        await driver.sleep(1000);
        const selectCondition = await massUpdateElementsObj.findSelectConditionDropdown(driver);
        await selectCondition.click();
        await driver.sleep(1000);
        //select 'not empty' condition
        const listCondition = await massUpdateElementsObj.findListCondition(driver, 9);
        ...
        await driver.sleep(1000);
        const massUpdateButton = await formElementsObj.findElementByVisibleText(driver,screenshotPath,'button','Update')
        ...
        for (let i = 0; i < dataTable.rawTable.length; i++) {
            ...
            if (fieldName == 'select field01') {
                ...
                const nameField = await formElementsObj.findElementById(driver,screenshotPath,'name','Product Name');
        ...
        const productName = await addProductElementsObj.clickOnProductName(driver, 'Product3');
...
When('system should display {string} button as disabled if user have not checked any checkbox',async function(massUpdateButton) {
    try {
        const actionsButton = await formElementsObj.findElementByVisibleText(driver,screenshotPath,'button',' Actions ');
        await actionsButton.click();
        await driver.sleep(1000);
        ...
        await searchButton.click();
        await driver.sleep(1000);
        //check disability of 'Update' button due to uncheck of product
        const updateButton = await driver.findElement(By.xpath(`//button[text()="${massUpdateButton}"]`));
        await driver.executeScript("arguments[0].scrollIntoView();", updateButton);
        await driver.sleep(2000);
        //screenshot 'disabled update button'
        await screenshotObj.takeScreenshot(driver, screenshotPath + 'disabledUpdateButton.png');
        const updateButtonDisability = !!await updateButton.getAttribute('disabled');
        console.log("As no product is checked,is update button is disabled: " + updateButtonDisability);
        if (updateButtonDisability === true) {
            console.log("As update button is disabled due to user have not checked any product,so test case has been passed");
        } else {
            await assert.fail("As update button is displayed even user have not checked any product,so test case has been aborted");
        }
...
When('user clicks on cancel button it should terminate update process',async function() {
    try {
        const actionsButton = await formElementsObj.findElementByVisibleText(driver,screenshotPath,'button',' Actions ');
        await actionsButton.click();
        await driver.sleep(1000);
        ...
        if (currentPageURL.endsWith('app/products/list')) {
            console.log("After clicking on 'Cancel' button mass update process terminated and user redirected to product listing page,so test case has been passed");
        } else {
            await assert.fail("After clicking on 'Cancel' button mass update process is unterminated and user does not redirected to product listing page,so test case has been aborted");
        }
    } catch(err) {
        await driver.navigate().refresh();
        await driver.sleep(5000);
...
//----------Case 6: As a User, system should give me a validation message when any criteria are not selected------------

When('system should give a validation {string} when any criteria is not selected',async function(expectedValidation) {
    try {
        const actionsButton = await formElementsObj.findElementByVisibleText(driver,screenshotPath,'button',' Actions ');
        await actionsButton.click();
        await driver.sleep(1000);
