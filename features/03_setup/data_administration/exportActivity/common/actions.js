...
async function openExportActivityPage(driver,screenshotPath) {
    ...
    //will verify whether the export activity page found or not
    try{
        await driver.wait(until.urlContains('app/setup/export/detail?id=2'),10000);
    }catch(err){
        await screenshotObj.takeScreenshot(driver,screenshotPath+'exportActivityPage_NotFound.png');
        await assert.fail('Due to the export activity page is not found, the test case has been failed. Error Details: \''+err+'\' Screenshot Name: \''+screenshotPath+'exportActivityPage_NotFound.png\'.');
    }
    console.log('The export activity page has been opened successfully...');
}exports.openExportActivityPage=openExportActivityPage;