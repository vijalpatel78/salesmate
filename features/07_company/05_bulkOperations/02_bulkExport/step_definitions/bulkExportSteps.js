...
//----------Case 3: Verify that it should allow me to export selected companies from the companies listing page-----------

When('Verify that it should allow me to export selected companies from the companies listing page',async function() {
    try {
        ...
        const file = './downloadedFile/Company-export-data.csv';
        try {
            ...
            await screenshotObj.takeScreenshot(driver, screenshotPath + 'companyFile_NotDownloaded.png');
            await assert.fail(err);
        }
        console.log('As "Company-export-data.csv" file is downloaded successfully in the "./downloadedFile" folder,so test case has been passed');
    } catch(err) {
