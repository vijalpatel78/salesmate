...
const screenshotPath = './features/09_activity/04_deleteActivity/screenshots/';
const pageNavigationObj = require('../../01_moduleAccessibility/common/actions');
const formElementsObj = require('../../../00_common/webElements/formElements');
const moduleElementsObj = require('../../../00_common/webElements/moduleElements');

//---------Case 1: Verify, user is not able to see Delete activity option if user does not have Delete activity rights-----------

Then('delete activity button is not visible and log in through {string}',async function(adminUser) {
    try {
...
//------------Case 2: Verify, user is able to see Delete activity option if user have Delete activity rights--------------

Then('delete activity button is visible and log in through {string}',async function(adminUser) {
    try {
        const activityModule = await moduleElementsObj.findModuleIcon(driver,'icon-ic_activity');
        await activityModule.click();
        await driver.sleep(2000);
...
//-------------------------------case 3: Verify, the user is able to delete a activity----------------------------------

When('Verify, the user is able to delete a activity and verify {string}',async function(expectedNotification) {
    try {
        //get activity lists count before deleting contact
        const activityLists = await driver.findElements(By.xpath('//a[@class="entity-show-link text-ellipsis"]'));
        const activitiesCountBeforeDeleting = await activityLists.length;
        const contactCheckbox = await moduleElementsObj.findContactCheckbox(driver,1);
        await contactCheckbox.click();
        await driver.sleep(1000);
        ...
        await driver.sleep(3000);
        await pageNavigationObj.comeBackToActivityListingPage(driver);
        await driver.sleep(1000);
        //get contact lists count before adding contact details
        const activityCount = await driver.findElements(By.xpath('//a[@class="entity-show-link text-ellipsis"]'));
        const activityCountAfterDeleting = await activityCount.length;
        if((activitiesCountBeforeDeleting)-1 === activityCountAfterDeleting) {
            console.log("As activities length is decreased by (X-1) after performing delete operation,so test case has been passed");
        } else {
            await assert.fail("As activities length is not decreased by (X-1) even after performing delete operation,so test case has been aborted");
        }
    } catch(err) {
        await screenshotObj.takeScreenshot(driver,screenshotPath+'activity_NotDeleted.png');
