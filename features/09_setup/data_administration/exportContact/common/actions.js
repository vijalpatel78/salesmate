...
async function openExportContactPage(driver,screenshotPath) {
    ...
    if(exportDataTab.length > 0){
        ...
        //will click on 'Export Contact' tab
        ...
        await exportDataTab[0].click();
        await driver.sleep(1000);
        //will click on the 'Export Contact' tab
        await exportContactElementsObj.findContactsExportModule(driver);
        await driver.sleep(2000);
    }
    await driver.sleep(2000);
    //will verify whether the export contact page found or not
    try{
        await driver.wait(until.urlContains('app/setup/export/detail?id=1'),10000);
    }catch(err){
        await screenshotObj.takeScreenshot(driver,screenshotPath+'exportContactPage_NotFound.png');
        await assert.fail('Due to the export contact page is not found, the test case has been failed. Error Details: \''+err+'\' Screenshot Name: \''+screenshotPath+'exportContactPage_NotFound.png\'.');
    }
    console.log('The export contact page has been opened successfully...');
}exports.openExportContactPage=openExportContactPage;