...
const openExportDealPageObj = require('../common/actions');
const performSalesmateLoginObj = require('../../../../00_common/actions/performSalesmateLogin');
const clearFieldDataObj = require('../../../../00_common/actions/fieldActions/clearFieldData');
const screenshotObj = require('../../../../00_common/actions/browserActions/takeScreenshot');
const driver = require('../../../../../cucumber_config/cucumber_config').driver;
const screenshotPath = './features/03_setup/data_administration/exportDeal/screenshots/';
const exportDealElementsObj = require('../common/exportDealElements');
const exportContactElementsObj = require('../../../data_administration/exportContact/common/exportContactElements');
...
let companySingularName, contactSingularName, associatedContactsState = 'no', enablingLastModifiedBy = 'no';
let disablingLastModifiedBy = 'no',lastNameState = 'no';

Given('the {string} is on export deal page',async function(user) {
    const currentPageTitle = await driver.getTitle();
    const currentPageURL = await driver.getCurrentUrl();

    //will check that the user is on which page
    if(await currentPageURL.includes('app/setup/export/detail?id=4')){
         ...
         *  then the process to open export deal page will be started from by performing the Salesmate login
        */

        //will do Salesmate login with the specified user's credentials
        await performSalesmateLoginObj.performSalesmateLogin(driver,screenshotPath,user,'the {string} is on export deal page');
        //will open the export deal page
        await openExportDealPageObj.openExportDealPage(driver);
    }
         ...
         *  then the process to open export deal page will be started from by opening the Salesmate login page
        */

        //will open the Salesmate login page
        await openSalesmatePageObj.openSalesmateLoginPage(driver,screenshotPath,'the {string} is on export deal page');
        //will do Salesmate login with the specified user's credentials
        await performSalesmateLoginObj.performSalesmateLogin(driver,screenshotPath,user,'the {string} is on export deal page');
        //will open the export deal page
        await openExportDealPageObj.openExportDealPage(driver);
        ...
        //as the user is logged in, it will open the export deal page
        await openExportDealPageObj.openExportDealPage(driver);
    }
});

//------------------------------Case 1 : the system should display dynamic module name of deal---------------------------------------

When('the system should display dynamic module name of deal',async function() {
    try {
        ...
        const dealPluralName = await driver.findElement(By.id('pluralName')).getAttribute('value');
        console.log("Deal Plural Name: " + dealPluralName);
        //will open the 'Setup' page
        await openSalesmatePageObj.openSetupPage(driver, screenshotPath);
        ...
        await driver.sleep(2000);
        const exportDataDealPluralName = await driver.findElement(By.xpath(`//div[text()=" ${dealPluralName} "]`)).getText();
        console.log("Export Data deal Plural Name: " + exportDataDealPluralName);
        await driver.manage().setTimeouts({implicit: 2000});
        const exportDataDeal = await driver.findElements(By.xpath(`//div[text()=" ${dealPluralName} "]`));
        const exportDataDealPluralNameLength = await exportDataDeal.length;
        if (exportDataDealPluralNameLength > 0) {
            console.log("As export data deal plural name " + exportDataDealPluralName + " in export data exists,so test case has been passed");
        } else {
            await assert.fail("As export data deal plural name " + exportDataDealPluralName + " in export data does not exist,so test case has been aborted");
        }
        await exportDealElementsObj.findDealExportModule(driver);
        await driver.sleep(3000);
        const dealHeaderName = await driver.findElement(By.xpath(`//h2[@class="m-b-xs"]`)).getText();
        console.log("Deal Header Name: " + dealHeaderName);
        await driver.manage().setTimeouts({implicit: 2000});
        const dealLowerCase = await exportDataDealPluralName.toLowerCase();
        const dealHeaderElements = await driver.findElements(By.xpath(`//h2[contains(.,"${dealLowerCase}")]`));
        const dealHeaderLength = await dealHeaderElements.length;
        if (dealHeaderLength > 0) {
            console.log("As header name of exports deal page " + dealHeaderName + " is displayed,so test case has been passed");
        } else {
            await assert.fail("As header name of exports deal page " + dealHeaderName + " is not displayed,so test case has been aborted");
        }
        await driver.manage().setTimeouts({implicit: 2000});
        const dealsLowerCase = await dealPluralName.toLowerCase();
        const moduleHeaderElements = await driver.findElements(By.xpath(`//h2[contains(.,"${dealsLowerCase}")]`));
        const moduleHeaderLength = await moduleHeaderElements.length;
        if (moduleHeaderLength > 0) {
            console.log("As module header name of exports deal page " + dealHeaderName + " is displayed,so test case has been passed");
        } else {
            await assert.fail("As module header name of exports deal page " + dealHeaderName + " is not displayed,so test case has been aborted");
        }
    } catch(err) {
...
//-------------Case 2: Verify, the system should display custom view list that displayed on the deal list page------------------------------------

When('the system display custom view list that displayed on the deal list page',async function() {
    try {
        await driver.sleep(2000);
        await exportDealElementsObj.findDealIcon(driver);
        await driver.sleep(3000);
        await driver.findElement(By.xpath('//deal-list//sm-custom-popover/div/a')).click();
        await screenshotObj.takeScreenshot(driver, screenshotPath + 'dealPageCustomView.png');
        await driver.sleep(1000);
        const dealPageCustomViewValue = await driver.findElements(By.xpath("//div[@id='popover1-body']/div/div[3]/div/ul[1]/li"));
        const dealPageCustomViewValueLength = await dealPageCustomViewValue.length;
        console.log(dealPageCustomViewValueLength);
        console.log("Below are deal page custom view values:");
        for (let i = 1; i <= dealPageCustomViewValueLength; i++) {
            const dealPageCustomViewValue = await driver.findElement(By.xpath(`//div[@id='popover1-body']/div/div[3]/div/ul/li[${i}]/a`));
            const dealPageCustomViewText = await dealPageCustomViewValue.getText();
            console.log(dealPageCustomViewText);
        }
        //will open the 'Setup' page
        await openSalesmatePageObj.openSetupPage(driver, screenshotPath);
        //will find the 'Export Data' tab
        ...
        await driver.sleep(2000);
        await screenshotObj.takeScreenshot(driver, screenshotPath + 'dealContactPageCustomView.png');
        await driver.manage().setTimeouts({implicit: 2000});
        console.log("Below are export deal page custom view values:");
        const exportDealPageCustomViewValue = await driver.findElement(By.name("customView")).getText();
        console.log(exportDealPageCustomViewValue);
        const exportDealPageCustomView = await driver.findElements(By.xpath("//ul[@role='tree']/li"));
        const exportDealPageCustomViewLength = await exportDealPageCustomView.length;
        console.log(exportDealPageCustomViewLength);
        if (dealPageCustomViewValueLength === exportDealPageCustomViewLength) {
            console.log("As deal page custom view length: " + dealPageCustomViewValueLength + " and export deal page custom view length: " + exportDealPageCustomViewLength + " are equal,so test case has been passed");
        } else {
            await assert.fail("As deal page custom view: " + dealPageCustomViewValueLength + " and export deal page custom view length: " + exportDealPageCustomViewLength + " are not equal,so test case has been aborted");
        }
...
//----------------------------Case 3: Verify, the user is able to export the deal data-------------------------------------

When('the user is able to export the deal data and verify {string}',async function(expectedNotification) {
    try {
        await driver.sleep(3000);
        ...
        await driver.sleep(1000);
        await exportContactElementsObj.findSelectAllLink(driver);
        await exportContactElementsObj.findModuleTab(driver, 'Deal');
        await driver.sleep(1000);
        await exportContactElementsObj.findSelectAllLink(driver);
        ...
        await driver.sleep(1000);
        console.log("As user is able to export the deal data,so test case has been passed");
    } catch(err) {
        await driver.navigate().refresh();
        await driver.sleep(5000);
        await assert.fail(err);
...
//---------------------Case 4: Verify, the user is able to search the deal fields--------------------------------------

When('the user is able to search the deal fields',async function(dataTable) {
    try {
        await driver.sleep(2000);
        //will open the 'Setup' page
        await openSalesmatePageObj.openSetupPage(driver, screenshotPath);
...
//---------------------------Case 5: Verify, the user is able to manage the fields for the deal-----------------------------

When('the user is able to manage the fields for the deal',async function(dataTable) {
    try {
        await driver.sleep(3000);
        await driver.executeScript("return $('.wrapper-lg').animate({ scrollTop: 3000 }, 0);");
        await driver.sleep(2000);
...
Then('check data {string} should be displayed under selected fields of export deal',async function(expectedSelectField) {
    try {
        await driver.sleep(2000);
        await driver.manage().setTimeouts({implicit: 2000});
...
When('the user is able to manage the fields for the export deal',async function(dataTable) {
    try {
        //will travel provided fields and data list
        for (let i = 0; i < dataTable.rawTable.length; i++) {
...
Then('uncheck data {string} should not be displayed under selected fields of export deal',async function(expectedSelectField) {
    try {
        await driver.sleep(2000);
        await driver.manage().setTimeouts({implicit: 2000});
        const lastModifiedByElements = await driver.findElements(By.xpath(`//li[contains(text(),"${expectedSelectField}")]`));
...
When('the company module name should be displayed on the company fields in export deal',async function(dataTable) {
    try {
        await driver.sleep(1000);
        //will open the 'Setup' page
        await openSalesmatePageObj.openSetupPage(driver, screenshotPath);
        ...
        for (let i = 0; i < dataTable.rawTable.length; i++) {
            ...
            if (fieldName == 'search field data') {
                ...
                if (associatedContactsState == 'enable' || associatedContactsState == 'disable') {
                    ...
                    const associatedContactsToggle = await exportDealElementsObj.findAssociatedContactsCheckbox(driver);
...
Then('check data {string} should be displayed under selected fields in export deal',async function(expectedSelectField) {
    try {
        await driver.sleep(2000);
        await driver.manage().setTimeouts({implicit: 2000});
...
When('the contact module name should be displayed on the contact fields in export deal',async function(dataTable) {
    try {
        await driver.sleep(1000);
        //will open the 'Setup' page
        await openSalesmatePageObj.openSetupPage(driver, screenshotPath);
        ...
        for (let i = 0; i < dataTable.rawTable.length; i++) {
            ...
            if (fieldName == 'search field data') {
                ...
                if (lastNameState == 'enable' || lastNameState == 'disable') {
                    ...
                    const lastNameToggle = await exportDealElementsObj.findLastNameCheckbox(driver);
...
Then('check data {string} should be displayed under selected fields division in export deal',async function(expectedSelectField) {
    try {
        await driver.sleep(2000);
        await driver.manage().setTimeouts({implicit: 2000});
        const lastNameField = await driver.findElement(By.xpath(`//li[contains(text(),"${expectedSelectField}")]`));
        await driver.executeScript("arguments[0].scrollIntoView();", lastNameField);
        await driver.wait(until.elementIsVisible(lastNameField));
        const lastNameFieldText = await driver.findElement(By.xpath(`//li[contains(text(),"${expectedSelectField}")]`)).getText();
        console.log("After check the checkbox of last name, the field displaying under select fields is: " + lastNameFieldText);
        const lastNameFieldElements = await driver.findElements(By.xpath(`//li[contains(text(),"${expectedSelectField}")]`));
        const lastNameFieldLength = await lastNameFieldElements.length;
        if (lastNameFieldLength > 0) {
            console.log("As selected field " + lastNameFieldText + " is found after check " + lastNameFieldText + " checkbox,so test case has been passed");
        } else {
            await assert.fail("As selected field " + lastNameFieldText + " is not found after check " + lastNameFieldText + " checkbox,so test case has been aborted");
        }
        if (lastNameFieldText.includes(contactSingularName)) {
            console.log("As selected field " + lastNameFieldText + " contains " + lastNameFieldText + " ,so test case has been passed");
        } else {
            await assert.fail("As selected field " + lastNameFieldText + " does not contains " + lastNameFieldText + " ,so test case has been aborted");
        }
    } catch(err) {
        await driver.navigate().refresh();
        await driver.sleep(5000);
